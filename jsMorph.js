/*!
 * jsMorph JavaScript Library v0.5.0
 * http://jsMorph.com/
 *
 * Copyright 2011, Peter Dematt茅
 * Dual licensed under the MIT or GPL Version 2 licenses.
 * http://jsMorph.com/license
 *
 * Date: Mon Nov 28 14:12:28 2011 -0100
 */
 var jsMorph = function (obj, prop, params, ease, onMorphInit, onMorph, onMorphEnd) {
	var IE = !document.getElementsByClassName,
		getInitProp = (function (obj, prop, duration, speed, onMorphInit, jsMorph) {
			...
			units = function (obj) { // this usualy only happens with IE and OPERA
				var units = {},
					oD,
					objStyle = obj.style,
					sniff = document.createElement('div'),
					sniffStyle = sniff.style;
		...
		for (var n in prop) { // faster if only px
			...
			color = n.match(/color/);
			if (!(n.replace(/\-(\w)/g, function(){return arguments[1].toUpperCase()}) in objStyle) && !opacity) {delete(prop[n]);continue} // take everything out that can't be rendered anyhow.. // how long does this take? Is it worth it? 觉得没必要
			...
			if (!cS[1] && n.match(/^(height|width)/)) { // IE and OPERA  // the following code has to go different %&搂#*?uuaaaaahhhh
	...
	anim = function (time, timer, backwards, frames, initProps, objs, jsMorph, IE) {
		...
		if (cont){
			jsMorph.timer = window.setTimeout(function(){anim(timer(), timer, backwards, ++frames, initProps, objs, jsMorph, IE)}, jsMorph.speed);
		}else {
			jsMorph.timer = null;
	...
	this.concat = function (obj, prop, params, ease, onMorphInit, onMorph, onMorphEnd) {
		if (!obj.pop && !obj.item) obj = [obj];	//如果不是数组或者dom队列
		if (!params) params = {};
		for (var n = obj.length; n--;) {// make it faster feeding initProp with an array??
			if (typeof obj[n] == 'string') obj[n] = document.getElementById(obj[n]);	//如果是字符串就当是dom的id
